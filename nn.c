 ...
 *  Stop when we fail less than 5 % of validation tests.
 *  YOU MAY WISH TO CHANGE THIS.
 *  Valid range: 0, ...
 */
#define MIN_ERRORS  0.025
 ...
 *  Valid range: 0.00, ..., 1.00
 */
#define TRAINING_PROPORTION  0.80



/*
 *  Fraction of the training data to use for training.
 *  The rest is used for validation.
 *  YOU MAY WISH TO CHANGE THIS.
 *  Valid range: 0.00, ..., 1.00
 */
#define REAL_TRAINING_PROPORTION  0.75
...
void    splitData( int n_input, int n_output, dd_type *all_data,
           dd_type *training_data, dd_type *test_data, dd_type *validation_data );
void    trainNetwork( FILE *log_file, nn_type *nn, dd_type *training_data, dd_type *validation_data );
int    testNetwork(  FILE *out_file, nn_type *nn, dd_type *test_data, bool validation );
...
main( int argc, char *argv[] )
{
    ...
    nn.initial_learning_rate = atof( argv[1] );
    nn.k             = atof( argv[2] );
	...
	&all_data, &training_data, &test_data, &validation_data );
    trainNetwork( log_file, &nn, &training_data, &validation_data );
    testNetwork(  log_file, &nn, &test_data, false );
    ...
    dd_type *validation_data )
{
    ...
    /*
     *  Use 80% of the training data for training.
        Use the rest for validation_data
     */
    int initialTrainingDataSize = training_data->n;
    training_data->n = REAL_TRAINING_PROPORTION * training_data->n;
    remove = initialTrainingDataSize - training_data->n;
    validation_data->n = remove;

    for( i = 1; i <= remove;  ) {
    	k = random() % all_data->n;
    	/* mark k^th example as not in training data if it's not
    	   already marked */
    	if( training_data->x[k][0] != -1.0 && training_data->x[k][0] != -2.0 ) { //we have found actual training data, now make it validation data
    	    training_data->x[k][0] = -2.0;
    	    i++;
    	}
    }
    ...
    for( i = 0; i < all_data->n; i++ ) {
    	if( training_data->x[i][0] != -1.0 && training_data->x[i][0] != -2.0 ) {
    	    for( j = 0; j <= n_input; j++ ) {
    	    ...
    	    k++;
    	} else if (training_data->x[i][0] == -2.0) {
            for( j = 0; j <= n_input; j++ ) {
    	        validation_data->x[m][j] = all_data->x[i][j];
    	    }
    	    for( j = 1; j <= n_output; j++ ) {
    	        validation_data->y[m][j] = all_data->y[i][j];
    	    }
    	    m++;
        } else {   //test data has the training_data->x[i][0] field set to -1
    	    for( j = 0; j <= n_input; j++ ) {
    ...
    fprintf( stdout, "Number of training examples: %d\n", k );
    fprintf( stdout, "Number of validation examples: %d\n", m );
    fprintf( stdout, "Number of test examples: %d\n", l );
...
void adjustLearningRateStepDecay( nn_type *nn, int epoch){
    static int numAdjustments = 0;
    int x = 200;
    int temp = epoch % x;

    if (temp > numAdjustments){
        nn->learning_rate /= 2.0;
        numAdjustments++;
    }
}

void adjustLearningRateSearchAndConverge( nn_type *nn, int epoch){
    //do not update the learning_rate for the first x epochs
    double x = 75;
    nn->learning_rate = (nn->initial_learning_rate / (1 + (epoch/ x)));
}
...
adjustWeights( nn_type *nn, double x[], double y[] )
{
    ...
    for( i = 0; i <= nn->n_hidden; i++ )
    for( j = 1; j <= nn->n_output; j++ ) {
	nn->W2[i][j] = nn->W2[i][j] + (nn->learning_rate + 0.05)*E2[j]*nn->h[i];
    }
    ...
    for( i = 0; i <= nn->n_input; i++ )
    for( j = 1; j <= nn->n_hidden; j++ ) {
	nn->W1[i][j] = nn->W1[i][j] + (nn->learning_rate + 0.05)*E1[j]*x[i];
    }
}


void
trainNetwork( FILE *log_file, nn_type *nn, dd_type *data, dd_type *validation_data )
{
    int     i, j, p;
    int     epoch, errors, validation_errors;
    double  drand48();
    void    determineOutput();
    void    adjustWeights();
    void    adjustLearningRateStepDecay();
    void    adjustLearningRateSearchAndConverge();
    //int     testNetwork();
    ...
    do {
    	...
    	for( p = 0; p < data->n; p++ ) {
    	     ...
    	     */
    	    if( errors ) {
                //adjustLearningRateStepDecay(nn, epoch);
                adjustLearningRateSearchAndConverge(nn, epoch);
        		adjustWeights( nn, data->x[p], data->y[p] );
    	    }
        }

        validation_errors = testNetwork(log_file, nn, validation_data, true);

        fprintf( log_file, "Epoch: %3d,  ", epoch );
        fprintf( log_file, "Number of classification errors: %d (/%d),",
			errors, data->n );
        fprintf( log_file, " Validation data error = %d (/%d)\n",
        	validation_errors, validation_data->n );
	     fflush( log_file );

    } while ((validation_errors > (MIN_ERRORS * validation_data->n)) && (epoch < MAX_EPOCH));
...
int
testNetwork( FILE *log_file, nn_type *nn, dd_type *data, bool validation )
{
    int     j, p;
    int     errors;
    void    determineOutput();
